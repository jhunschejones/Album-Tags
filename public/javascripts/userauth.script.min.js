console.log('The custom script for the userauth page is running');function hideDOMelement(elementId){try{let element=document.getElementById(elementId)
    ...
    var whatConnectionsToShow=sessionStorage.getItem('connections');if(whatConnectionsToShow=="My Connections"||whatConnectionsToShow==undefined){sessionStorage.setItem('connections','My Connections');filterDisplayedConnections()}else if(whatConnectionsToShow=="All Connections"){sessionStorage.setItem('connections','All Connections');allConnectionsNoFilter()}}}else{noUserSignedIn()}});function noUserSignedIn(){try{logInMessage.innerHTML='<div class="container-fluid please_log_in"><p class="text-danger">Please sign in to access this feature</p> <button onclick="logIn()" class="btn btn-danger" id="login_link">Log In</button></div>'}catch(error){}
