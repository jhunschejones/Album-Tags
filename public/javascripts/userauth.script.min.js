console.log('The custom script for the userauth page is running');function hideDOMelement(elementId){try{var element=document.getElementById(elementId)
    ...
    var whatConnectionsToShow=sessionStorage.getItem('connections');if(whatConnectionsToShow=="My Connections"||whatConnectionsToShow==undefined){sessionStorage.setItem('connections','My Connections');filterDisplayedConnections();}else if(whatConnectionsToShow=="All Connections"){sessionStorage.setItem('connections','All Connections');allConnectionsNoFilter()}}}
    var sessionId=JSON.parse(sessionStorage.getItem('sessionId'));var sessionLogin=false
    if(sessionId!=null){sessionLogin=true;console.log("Session storage login");signInFunctionality(sessionId);}
    firebase.auth().onAuthStateChanged(function(user){if(sessionLogin===false){if(user){sessionStorage.setItem('sessionId',JSON.stringify(user));console.log("Firebase login")
    ...
    function filterDisplayedTags(){try{var anyTagsOnPage=false
    tagsForThisAlbum=$(".album_details_tags")
    for(let index=0;index<tagsForThisAlbum.length;index++){var thisTag=tagsForThisAlbum[index];if($(thisTag).hasClass(`author-${userID}`)){$(thisTag).show()
    anyTagsOnPage=true}else{$(thisTag).hide()}}
    if(anyTagsOnPage==true){$(".tag_search_button").show()
    $('#tag_results').show()
    ...
    function allTagsNoFilter(){try{var anyTagsOnPage=false
    tagsForThisAlbum=$(".album_details_tags")
    if(tagsForThisAlbum.length>0){anyTagsOnPage=true
    for(let index=0;index<tagsForThisAlbum.length;index++){var thisTag=tagsForThisAlbum[index];$(thisTag).show()}}
    if(anyTagsOnPage==true){$(".tag_search_button").show()
    $('#tag_results').show()
    $('#tag_results_message').html('')}else{$(".tag_search_button").hide()
    $('#tag_results').hide()
    $('#tag_results_message').html('<small class="text-primary">There are currently no tags for this album!</small>');}}catch(error){}}
    function filterDisplayedConnections(){try{var anyConnectionsOnPage=false
    connectionsForThisAlbum=$(".connection")
    for(let index=0;index<connectionsForThisAlbum.length;index++){var connection=connectionsForThisAlbum[index];if($(connection).hasClass(`author-${userID}`)){$(connection).show()
    anyConnectionsOnPage=true}else{$(connection).hide()}}
    if(anyConnectionsOnPage==true){$('#connection_results').show()
    ...
    function allConnectionsNoFilter(){try{var anyConnectionsOnPage=false
    connectionsForThisAlbum=$(".connection")
    if(connectionsForThisAlbum.length>0){anyConnectionsOnPage=true
    for(let index=0;index<connectionsForThisAlbum.length;index++){var thisConnection=connectionsForThisAlbum[index];$(thisConnection).show()}}
    if(anyConnectionsOnPage==true){$('#connection_results').show()
