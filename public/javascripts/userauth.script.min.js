console.log('The custom script for the userauth page is running');function hideDOMelement(elementId){try{let element=document.getElementById(elementId)
    ...
    var config={apiKey:"AIzaSyD1Hts7zVBvDXUf-sCb89hcPesJkrUKyUc",authDomain:"album-tag-auth.firebaseapp.com",databaseURL:"https://album-tag-favorites.firebaseio.com/",projectId:"album-tag-auth",storageBucket:"album-tag-auth.appspot.com",messagingSenderId:"1048555394172"};const app1=firebase.initializeApp(config);const app2=firebase.initializeApp({apiKey:"AIzaSyD1Hts7zVBvDXUf-sCb89hcPesJkrUKyUc",databaseURL:"https://album-tag-connections.firebaseio.com/",projectId:"album-tag-auth"},'app2');var database1=firebase.database(app2);const allowedUsers=["joshjones103@gmail.com","znoffy5@gmail.com","devon.curry891@gmail.com","milesjohnsonmusic@gmail.com","austinhuelsbeck@gmail.com"];var loginButton=document.getElementById("login_button");var loginButton2=document.getElementById("full_menu_login_button")
    var logoutButton=document.getElementById("logout_button");var logoutButton2=document.getElementById("full_menu_logout_button")
    var logInMessage=document.getElementById("log_in_message");var loader=document.getElementById("loader");var userEmail="";var userName="";var user=firebase.auth().currentUser;var userID;var dbRefrence;firebase.auth().onAuthStateChanged(function(user){if(user){$('.hide_when_logged_in').addClass('hide_me');$('.hide_when_logged_out').removeClass('hide_me');showDOMelement("full_menu_login_logout_container");userEmail=user.email;userName=user.displayName;userID=user.uid;try{updateFavoriteAlbums()}
    catch(error){}
    try{updateConnectedAlbums()}catch(error){}
    try{populateUserData()}catch(error){}
    if(allowedUsers.indexOf(userEmail)>-1){tagUpdatePermissionsGranted();filterDisplayedTags()}
    else{tagUpdatePermissionsGranted();filterDisplayedTags()}}else{noUserSignedIn()}});function noUserSignedIn(){try{logInMessage.innerHTML='<div class="container-fluid please_log_in"><p class="text-danger">Please sign in to access this feature</p> <button onclick="logIn()" class="btn btn-danger" id="login_link">Log In</button></div>'}catch(error){}
    ...
    function tagUpdatePermissionsGranted(){$("#update_button_container").html('<a href="" onclick="goToUpdatePage()" class="btn btn-sm btn-primary update_button hide_when_logged_out">Update Tags</a>');$("#connection_button_container").html('<a href="" onclick="goToUpdatePage()" class="btn btn-sm btn-outline-secondary update_button hide_when_logged_out">Update Connections</a>');try{logInMessage.innerHTML=""}catch(error){}}
